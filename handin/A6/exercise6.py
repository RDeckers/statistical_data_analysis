...
# It took to long to run imo, so I decided to write a good part in c++ and just throw it in the interpreter :)
# however, I was deving it locally on my machine which has ROOT7 installed. Turns out ROOT5 is even worse at interpreting
# c++ than I remember and I don't have the time now to  get it to run on the grid. :\
# I hope you can grade this on a machine that has ROOT6+.
...
#
# fnc_combined.SetParameter(0,2.1)
# root_h = ROOT.TH1F("m", ";invariant mass (TeV);entries per bin", 60, 1,3)
...
(H1, H0) = generate_LLR_distribution(50000, 2.1, True)
H1_mean = H1.GetMean()
print H1_mean, " = H1 mean" # 2.8868
print H0.GetMean(), " = H0 mean" # -2.3788
plot_LLR_distribution(H1, H0, "LLR_dynamic")

H0_fit = data.ComputeH0()
(H1_fit, m_fit) = data.ComputeH1VariableMass(1.2, 2.8, 0.01)
(H1, H0) = generate_LLR_distribution(50000, m_fit, True)

H1_mean = H1.GetMean()
print H1_mean, " = H1 mean" # 2.8868
print H0.GetMean(), " = H0 mean" # -2.3788
plot_LLR_distribution(H1, H0, "LLR_dynamic_fit")

cdf_dy = make_cdf("cdf_dynamic", H0)
print 1.0 - cdf_dy.Interpolate(H1_mean), " = predicted dynamic p-value" #0.0073
print 1.0 - cdf_dy.Interpolate(H1_fit-H0_fit), " = measured dynamic p-value" # 0.00264800
print m_fit, " = m fit"
print H1_fit, " = H1 fit"
#
